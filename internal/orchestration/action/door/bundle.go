...
var (
	ErrUnknownExecutionBundle     = errors.New("door action: persisted execution bundle is not registered")
	ErrExecutionBundleMismatch    = errors.New("door action: persisted execution identity does not match registered bundle")
	ErrBundleOperationUnsupported = errors.New("door action: operation is not supported by execution bundle")
)

type executionBindings struct {
	unlockApprovalNode string
	reconcileNodes     map[string]struct{}
}

func (b executionBindings) supportsReconciliation(nodeID string) bool {
	_, ok := b.reconcileNodes[nodeID]
	return ok
}
...
func doorV1BundleSpec(deps Dependencies) (bundleSpec, error) {
	plan, err := CompilePlan()
	if err != nil {
		return bundleSpec{}, fmt.Errorf("compile door v1 plan: %w", err)
	}
	if plan.ID != PlanID || plan.Version != PlanVersion || plan.Digest != doorV1PlanDigest {
		return bundleSpec{}, fmt.Errorf(
			"door action: v1 plan identity drifted: got=%s/%s/%s want=%s/%s/%s",
	...
	return bundleSpec{
		...
		bindings: executionBindings{
			unlockApprovalNode: NodeApproveUnlock,
			reconcileNodes: map[string]struct{}{
				NodeApplyLock:   {},
				NodeApplyUnlock: {},
			},
...
func newBundleCatalog(
	...
	if host == nil {
		return nil, errors.New("door action: bundle catalog requires host")
	}
	if len(specs) == 0 {
		return nil, errors.New("door action: bundle catalog requires at least one bundle")
	}
	catalog := &bundleCatalog{byDigest: map[string]*executionBundle{}}
	for _, spec := range specs {
		if spec.plan == nil || spec.registry == nil {
			return nil, errors.New("door action: execution bundle is incomplete")
		}
		if _, exists := catalog.byDigest[spec.plan.Digest]; exists {
			return nil, fmt.Errorf("door action: duplicate bundle digest %s", spec.plan.Digest)
		}
		engine, err := host.Register(spec.plan, spec.registry, engineOptions...)
		if err != nil {
			return nil, fmt.Errorf("register door bundle %s/%s: %w", spec.plan.ID, spec.plan.Version, err)
		}
		bundle := &executionBundle{
			plan:     spec.plan,
			registry: spec.registry,
			bindings: cloneBindings(spec.bindings),
			engine:   engine,
		}
	...
	if catalog.active == nil {
		return nil, fmt.Errorf("door action: active bundle digest %s is not registered", activeDigest)
	}
	return catalog, nil
}
...
func (c *bundleCatalog) bundleForExecution(execution *adgo.Execution) (*executionBundle, error) {
	...
	if bundle.engine == nil {
		return nil, errors.New("door action: execution bundle engine is not registered")
	}
	return bundle, nil
}
...
func (s *Service) executionBundle(ctx context.Context, executionID string) (*executionBundle, *adgo.Execution, error) {
	if s == nil || s.production == nil || s.bundles == nil {
		return nil, nil, errors.New("door action: service is not open")
	}
...
func (s *Service) validatePersistedExecutions(ctx context.Context) error {
	if s == nil || s.production == nil || s.bundles == nil {
		return errors.New("door action: service is not open")
	}
	catalog, ok := s.production.Store.(adgo.ExecutionCatalog)
	if !ok {
		return errors.New("door action: durable store does not expose an execution catalog")
	}
	ids, err := catalog.ListExecutionIDs(ctx)
	if err != nil {
		return fmt.Errorf("list persisted door executions: %w", err)
	}
	for _, id := range ids {
		...
		if err != nil {
			return fmt.Errorf("load persisted door execution %s: %w", id, err)
		}
		if terminalExecution(execution.Status) {
			continue
		}
		if _, err := s.bundles.bundleForExecution(execution); err != nil {
			return fmt.Errorf("validate persisted door execution %s: %w", id, err)
		}
	}
	return nil
}
